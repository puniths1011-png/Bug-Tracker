...
export const registerUser = async (
  ...
  try {
    ...
    if (role && !ALLOWED_ROLES.includes(role)) {
      res
        .status(400)
        .json({ message: "Role must be one of: admin, developer, tester" });
      return;
    }
...
export const inviteUser = async (
  ...
  try {
    ...
    if (userExists) {
      res.status(400).json({ message: "User already existss" });
      return;
    }
    ...
    res.status(201).json({
      message: mailerReady
        ? "Invite sent"
        : "Invite created, but no real mailer is configured. Email preview details are in the backend console.",
